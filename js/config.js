/* ═══════════════════════════════════════════════════════════
   MicroVest v10 — js/config.js
   Supabase: zmyiaviafmmwpgxfvsbq
═══════════════════════════════════════════════════════════ */

/* ── Supabase ────────────────────────────────────────────────
   Values injected by Netlify Edge Function (inject-env.js).
...
const SUPA_OPTIONS = {
  ...
  global: {
    headers: { 'x-app-version': '10.0' },
  },
};

/* FIX: Guard — supabase library must be loaded before config.js */
if (typeof supabase === 'undefined') {
  console.error('[MV] Supabase SDK not loaded! Check script order in HTML.');
}
...
const RPC = {
  ...
  incXP       : (uid, xp)         => db.rpc('inc_xp',        { uid, xp_amount: xp }).catch(() => {}),
};

/* ── APP CONFIG ──────────────────────────────────────────── */
const MV = {
  APP_URL: (typeof '__APP_URL__' !== 'undefined' &&
            '__APP_URL__' && !('__APP_URL__').startsWith('__'))
    ? '__APP_URL__'
    : 'https://microvest.app',
  ...
  /* ── STAKING PLANS ────────────────────────────────────── */
  STAKING: [
    { id:'flex',    name:'Flexible',  icon:'💧', minAmount:50,   apy:0.12, lockDays:7,   color:'#60A5FA' },
    { id:'silver',  name:'Silver',    icon:'🥈', minAmount:100,  apy:0.24, lockDays:30,  color:'#94A3B8' },
    { id:'gold',    name:'Gold',      icon:'🥇', minAmount:500,  apy:0.36, lockDays:90,  color:'#EAB308' },
    { id:'plat',    name:'Platinum',  icon:'💠', minAmount:2000, apy:0.42, lockDays:180, color:'#A78BFA' },
  ],
  ...
  getBadge   (id)  { return this.BADGES.find(b => b.id === id); },
  getStake   (id)  { return this.STAKING.find(s => s.id === id); },
};

/* ── GLOBAL AUTH MONITOR ─────────────────────────────────── */
const _publicPages = ['login.html', 'forgot.html', 'index.html', 'onboarding.html'];
...
if (!_isPublic) {
  db.auth.onAuthStateChange((event, session) => {
    ...
    const onAuthPage = authPages.some(p => location.pathname.endsWith(p));
    if ((event === 'SIGNED_OUT' || (!session && event !== 'INITIAL_SESSION')) && !onAuthPage) {
      if (typeof UI !== 'undefined') UI.toast('Sesi tamat. Sila log masuk semula.', 'error', 3000);
      setTimeout(() => { location.href = location.pathname.includes('/admin/') ? '../login.html' : 'login.html'; }, 1500);
    }
